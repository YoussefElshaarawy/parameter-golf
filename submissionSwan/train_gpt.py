...
class Hyperparameters:
    ...
    distill_weight = float(os.environ.get("DISTILL_WEIGHT", 0.8))
    distill_temperature = float(os.environ.get("DISTILL_TEMPERATURE", 2.0))
    distill_every = int(os.environ.get("DISTILL_EVERY", 4))
    distill_seq_tokens = int(os.environ.get("DISTILL_SEQ_TOKENS", 256))
    distill_ema_decay = float(os.environ.get("DISTILL_EMA_DECAY", 0.995))
...
def main() -> None:
    ...
    if args.use_distill and world_size != 1:
        raise ValueError("USE_DISTILL currently supports only single-GPU runs")
    if world_size <= 0:
    ...
    if args.use_distill:
        teacher_model = copy.deepcopy(base_model).eval()
    ...
    log0(
        ...
        f" distill={int(args.use_distill)}"
    )
    ...
    if master_process:
        ...
        log0(
            ...
            f"distill_every={args.distill_every} "
            f"distill_seq_tokens={args.distill_seq_tokens} "
    ...
    while True:
        ...
        for micro_step in range(grad_accum_steps):
            ...
            if args.use_distill:
                ...
                targets = y.reshape(-1)
                ce_loss = F.cross_entropy(student_logits.reshape(-1, student_logits.size(-1)).float(), targets, reduction="mean")
                if args.distill_every > 0 and step % args.distill_every == 0 and teacher_model is not None:
                    distill_input = x[:, -min(x.size(1), args.distill_seq_tokens):]
                    with torch.no_grad():
                        teacher_logits = teacher_model.forward_logits(distill_input).float()
                    student_distill_logits = student_logits[:, -distill_input.size(1):].float()
                    temp = args.distill_temperature
                    distill_loss = F.kl_div(
                        F.log_softmax(student_distill_logits / temp, dim=-1),
                        F.softmax(teacher_logits / temp, dim=-1),
                        reduction="batchmean",
                    ) * (temp * temp)
                    loss = (1.0 - args.distill_weight) * ce_loss + args.distill_weight * distill_loss
                else:
                    loss = ce_loss
            else:
                with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True):
        ...
        if teacher_model is not None:
            ema_update_model(teacher_model, base_model, args.distill_ema_decay)
