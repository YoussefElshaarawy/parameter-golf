...
class Hyperparameters:
    ...
    fast_seq_frac = float(os.environ.get("FAST_SEQ_FRAC", "0.125"))
    fast_batch_frac = float(os.environ.get("FAST_BATCH_FRAC", "0.0625"))
    ...
    proxy_val_every = int(os.environ.get("PROXY_VAL_EVERY", 0))
    ...
    mtp_loss_weight = float(os.environ.get("MTP_LOSS_WEIGHT", 0.03))
...
class GPT(nn.Module):
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        if self.training and self.mtp_num_heads > 0 and self.mtp_loss_weight > 0.0:
            hidden = x.reshape(input_ids.size(0), input_ids.size(1), -1)
            mtp_loss_sum = hidden.new_zeros(())
            mtp_loss_count = 0
            for k, mtp_head in enumerate(self.mtp_heads):
                shift = k + 2
                valid_t = hidden.size(1) - shift
                if valid_t <= 0:
                    continue
                mtp_hidden = hidden[:, :valid_t, :].reshape(-1, hidden.size(-1))
...
def main() -> None:
    ...
    for mtp_head in base_model.mtp_heads:
        matrix_params.append(mtp_head.weight)
    ...
    log0(
        ...
        f" mtp_heads={args.mtp_num_heads}"
        f" mtp_loss_weight={args.mtp_loss_weight}"
    )
    ...
    load_state = base_model.state_dict()
    load_state.update(deq_state)
    base_model.load_state_dict(load_state, strict=True)
