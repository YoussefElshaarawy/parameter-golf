...
class Hyperparameters:
    ...
    fast_seq_frac = float(os.environ.get("FAST_SEQ_FRAC", "0.25"))
    fast_batch_frac = float(os.environ.get("FAST_BATCH_FRAC", "0.125"))
    fast_val_batch_frac = float(os.environ.get("FAST_VAL_BATCH_FRAC", "0.125"))
    fast_eval_batch_frac = float(os.environ.get("FAST_EVAL_BATCH_FRAC", "0.125"))
...
def main() -> None:
    ...
    default_grad_accum = 1 if world_size == 1 else max(1, 8 // world_size)
    grad_accum_steps = int(os.environ.get("GRAD_ACCUM_STEPS", str(default_grad_accum)))
    ...
    while True:
        ...
        should_proxy_validate = (
            not last_step
            and args.proxy_val_every > 0
            and step > 0
            and step % args.proxy_val_every == 0
        )
        should_validate = last_step or (args.val_loss_every > 0 and step % args.val_loss_every == 0) or should_proxy_validate
        if should_validate:
            torch.cuda.synchronize()
            ...
            val_loss, val_bpb = eval_val(
                ...
                max_eval_seqs=(args.proxy_val_max_seqs if should_proxy_validate and not last_step and args.val_loss_every == 0 else None),
            )
            if should_proxy_validate and not last_step and args.val_loss_every == 0:
                log0(
                    f"step:{step}/{args.iterations} proxy_val_loss:{val_loss:.4f} proxy_val_bpb:{val_bpb:.4f} "
    ...
    if master_process:
        log0(
            "submission_summary: "
            f"name=Relu3CleanResidualFast+NoBody+Int5MLP+SWA "
